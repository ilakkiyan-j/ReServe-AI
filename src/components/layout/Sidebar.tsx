...
const roleBadgeColors: Record<UserRole, string> = {
  ADMIN: "bg-purple-100 dark:bg-purple-500/15 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-500/30",
  CAFETERIA_MANAGER: "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-500/30",
  EVENT_MANAGER: "bg-sky-100 dark:bg-sky-500/15 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-500/30",
  RECIPIENT_ORGANIZATION: "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-500/30",
};
...
export const Sidebar: React.FC<SidebarProps> = ({ currentRole, activeTab, onTabChange }) => {
  ...
  return (
    <aside className="w-64 glass-panel border-r border-slate-200 dark:border-slate-800 flex flex-col min-h-[calc(100vh-65px)] sticky top-[65px] self-start bg-white/80 dark:bg-[#090d16]/80 backdrop-blur-md">
      {/* Role indicator */}
      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800/60">
        <div className={clsx(
          "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border text-xs font-semibold shadow-2xs",
      ...
      <nav className="flex-1 px-3 py-3 space-y-4 overflow-y-auto">
        {Object.entries(sections).map(([sectionKey, items]) => (
          <div key={sectionKey}>
            <p className="px-2 mb-1 text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      isActive
                        ? "bg-emerald-50 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 shadow-2xs"
